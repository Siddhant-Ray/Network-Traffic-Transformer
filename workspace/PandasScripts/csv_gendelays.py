...
def gen_packet_delay(input_dataframe1, input_dataframe2, path):
    ...
    ids_not_received = list(
        set(input_dataframe1["IP ID"].to_list())
    ...
    input_dataframe1["Delay"] = (
        input_dataframe2["Timestamp"] - input_dataframe1["Timestamp"]
    )
    # print(input_dataframe1.tail(), input_dataframe1.shape)
...
def main():
    ...
    parser.add_argument(
        "-mod", "--model", help="choose CC model for creating congestion", required=True
    )
    parser.add_argument(
        "-nsend",
        "--numsenders",
