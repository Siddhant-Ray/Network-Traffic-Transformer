...
/*   Referenced from https://github.com/pritam001/ns3-dumbell-topology-simulation
	...
	This script is for testing packet dropts on UDP flows only, no TCP.
...
NS_LOG_COMPONENT_DEFINE("congestion_udp");
...
void SingleFlow(bool pcap, std::string algo) {
    ...
    */
    ...
    while(i < 2){
        ...
        Ptr<OutputStreamWrapper> streamBytesInQueue = ascii.CreateFileStream("outputs/congestion_udp/bytesInQueue_router_"
                                                                             + std::to_string(i) + ".txt");
        ...
        Ptr<OutputStreamWrapper> streamPacketsInQueue = ascii.CreateFileStream("outputs/congestion_udp/packetsInQueue_router_"
                                                                             + std::to_string(i) + ".txt");
    ...
    while (j < 2){
        ...
        Ptr<OutputStreamWrapper> stream = ascii.CreateFileStream("outputs/congestion_udp/router" +
                                                                    std::to_string(j) + ".tr");
	...
	*/
	...
	NS_LOG_INFO("Measuring Performance of UDP.......");
	/********************************************************************
		One flow for each host and measure
		1) Throughput for long durations
		2) Size of queues at TC level, packet drops
	********************************************************************/
	...
	Ptr<OutputStreamWrapper> stream1CWND = asciiTraceHelper.CreateFileStream("outputs/congestion_udp/h1h2_singleflow.cwnd");
	Ptr<OutputStreamWrapper> stream1PD = asciiTraceHelper.CreateFileStream("outputs/congestion_udp/h1h2_singleflow.congestion_loss");
    ...
    Ptr<OutputStreamWrapper> streamRxDrops = ascii.CreateFileStream("outputs/congestion_udp/RxDrops_lrouter_"
    ...
    Ptr<OutputStreamWrapper> streamTxDrops = ascii.CreateFileStream("outputs/congestion_udp/TxDrops_lrouter_"
                                                                            + std::to_string(routerNum) + ".csv");
    ...
    Ptr<OutputStreamWrapper> streamRxEnds = ascii.CreateFileStream("outputs/congestion_udp/RxRevd_lrouter_"
                                                                            + std::to_string(routerNum) + ".csv");
    ...
    Ptr<OutputStreamWrapper> streamTxEnds = ascii.CreateFileStream("outputs/congestion_udp/TxSent_lrouter_"
                                                                            + std::to_string(routerNum) + ".csv");
    ...
    if (pcap)
    {
        p2pHR.EnablePcapAll("outputs/congestion_udp/pcap/singleflow");
	    p2pRR.EnablePcapAll("outputs/congestion_udp/pcap/RR_singleflow");
    }
